const getCompany = async () => {
  ...
  if (status !== "") {
      ...
      const company = response.data.conten;
      console.log(company);

        var tmp = "";
            for(let i = 0; i < company.size; i++){
                tmp +='<tr>';
                tmp    +=    '<td>'+company.content[i].code+'</td>';
                tmp        +=    '<td>'+company.content[i].name+'</td>';
                tmp        +=    '<td>'+company.content[i].email+'</td>';
                tmp        +=    '<td>'+company.content[i].phoneNumber+'</td>';
                tmp        +=    '<td id="'+company.content[i].status+'">'+company.content[i].status+'</td>';
                tmp        +=    '<td><button style="background: #30C0F0 0% 0% no-repeat padding-box;" onclick="detail('+company.content[i].id+')">Detail</button><button style="background: #D31212 0% 0% no-repeat padding-box;">Delete</button></td>';
                tmp        +='</tr>';
...
//   let pageIndex = Number(id.innerText);
//   var pageSize = document.getElementById("choose").value;

//     let payload = { pageSize: pageSize, pageIndex: pageIndex };

//     let data= await getCompany(payload);
//     console.log(data);

//     if (Number(pageSize) > Number(data.length)) {
//         document.getElementById('end').innerText = data.length + (data.number * data.size);
//     } else {
